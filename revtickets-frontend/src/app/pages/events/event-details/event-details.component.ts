...
@Component({
      ...
      <div class="event-header">
        ...
        <div class="details-section">
          ...
          <div class="event-meta">
            ...
            <p><mat-icon>schedule</mat-icon> {{event.dateTime | date:'medium'}}</p>
            <p *ngIf="event.durationMinutes"><mat-icon>timer</mat-icon> {{event.durationMinutes}} minutes</p>
        ...
        <div class="selected-day-shows" *ngIf="selectedDayShows.length > 0">
          ...
          <div class="shows-grid">
            <div *ngFor="let show of selectedDayShows" 
                 ...
                 [class.disabled]="show.disabled"
                 (click)="!show.disabled && bookShow(show.id)">
  ...
  styles: [`
    ...
    .show-time-slot.disabled {
      background: #444;
      border-color: #555;
      cursor: not-allowed;
      opacity: 0.5;
    }
    .show-time-slot.disabled .time {
      color: #888;
    }
    .show-time-slot.disabled .availability {
      color: #888;
    }
    .time {
      font-size: 14px;
...
export class EventDetailsComponent implements OnInit {
  ...
  private loadShows(eventId: string): void {
    this.eventService.getShowsForEvent(eventId).subscribe({
      next: (shows) => {
        this.shows = shows.filter(show => show.isActive !== false);
  ...
  private groupShowsByDay(): void {
    ...
    this.shows.forEach(show => {
      ...
      // Mark show as disabled if time has passed
      (show as any).disabled = showDate < now;
